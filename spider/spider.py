...
while start_page <= end_page:
    # 开始爬取数据
    logger.info(f"正在爬取第 {start_page}/{end_page} 页")
    ...
    if not articles:
        ...
        if failed_attempts > 3:
            logger.error("已连续失败 3 次，程序中止")
            os._exit(0)
        logger.warning("爬取数据失败，暂停 5 分钟后重试")
        time.sleep(300)
        continue
...
if os.path.exists(config_file):
    ...
    previous_count = 0
logger.info(f"共获取到 {len(data_list)} 条有效数据，上次 {previous_count} 条")
